...
public:
	...
	byte setClockWithCompileTimestamp(char &compileT);
...
//set the timer for asynchronous or synchronous mode
	...
	else if ((F_CPU == 8000000UL) || (F_CPU == 4000000UL))
	{ // prescaler set to 32
		TCCR2B &= ~(1 << CS22);
		TCCR2B |= ((1 << CS21) | (1 << CS20));
		prescaler = 32.0;
...
// Set the time with a compile timestamp __TIMESTAMP__
byte setClockWithCompileTimeStamp(char &compileT)
{
	// First we split by spaces
	//Sun Jan 31 15:17:02 2021
	char *ptr = strtok(compileT, " ");
	char *temparray[5];
	int i = 0;
